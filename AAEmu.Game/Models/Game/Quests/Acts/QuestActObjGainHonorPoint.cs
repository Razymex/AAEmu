...
/// <summary>
/// Progress: honor gained after accept (not the current wallet).
/// </summary>
public class QuestActObjGainHonorPoint(QuestComponentTemplate parentComponent) : QuestActTemplate(parentComponent)
{
    ...
    {
        Logger.Debug(
            "{0}({1}).RunAct: Quest {2}, Owner {3}, Honor {4}/{5}",
    ...
    {
        if (questAct.Template.ActId != ActId || e.Kind != QuestProgressStatKind.Honor || e.Amount <= 0)
            return;
